...
function RapportsEclairageUrgenceListContent() {
  ...
  const [modif, setModif] = useState<{ rapport: any; mode: 'technicien' | 'adresse' } | null>(null)
  ...
  async function supprimerRapport() {
    ...
    await fetch(`${API_URL}/api/rapports-eclairage-urgence/${supprimerId}/`, {
      method: 'DELETE',
  ...
  function chargerCompteurs() {
    const token = localStorage.getItem('access_token')
    fetch(`${API_URL}/api/rapports-eclairage-urgence/compteurs/`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  function charger(silent = false) {
    ...
    fetch(`${API_URL}/api/rapports-eclairage-urgence/?${params}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: NAVY }}>{t('titre_rapport_eclairage')}</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="flex gap-1 p-1 rounded-md border border-gray-100 bg-white w-full sm:w-auto">
          {([
            ...
            { key: 'ferme', label: `${t('fermes')} (${nbFermes})` },
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-md border border-gray-100 p-12 text-center">
          <i className="ti ti-bulb text-4xl text-gray-200" />
          ...
          <div className="divide-y divide-gray-50">
            {filtered.map(r => {
              ...
              return (
                <Link
                  key={r.id}
                  href={`/superviseur/rapports-eclairage-urgence/${r.id}`}
                ...
                >
                  ...
                  <span className="hidden md:inline text-xs text-gray-400">{r.nb_eclairages_urgence || 0} {t('col_unites').toLowerCase()}</span>
...
export default function SuperviseurRapportsEclairageUrgencePage() {
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center h-64">
        <div className="w-8 h-8 border-2 rounded-full animate-spin"
      ...
      <RapportsEclairageUrgenceListContent />
    </Suspense>
  )
}
