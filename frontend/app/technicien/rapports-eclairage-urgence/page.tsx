...
export default function TechnicienRapportsEclairageUrgencePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/rapports-eclairage-urgence/`, {
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{rapports.length} {rapports.length !== 1 ? t('rapports_pluriel') : t('rapport_singulier')}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {([
          ...
          { key: 'ouvert', label: t('ouverts') },
          { key: 'ferme', label: t('fermes') },
      ...
      {rapportsFiltres.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-md border border-gray-100">
          <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{ background: '#f8f9fa' }}>
            <i className="ti ti-bulb text-xl text-gray-300" />
          </div>
          <p className="text-gray-400 text-sm font-medium">{t('aucun_rapport')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {rapportsFiltres.map((r: any) => {
            ...
            return (
              <Link
                key={r.id}
                href={`/technicien/rapports-eclairage-urgence/${r.id}`}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-9 h-9 rounded-md flex-shrink-0 flex items-center justify-center" style={{ background: NAVY }}>
                      <i className="ti ti-bulb text-white text-sm" />
                    </div>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-400 pt-2 border-t border-gray-50">
                  ...
                  <span className="flex items-center gap-1">
                    <i className="ti ti-bulb text-gray-300" />
                    {r.nb_eclairages_urgence || 0} {t('col_unites').toLowerCase()}
                  </span>
                </div>
              </Link>
            )
